...
*   **Morning Push (07:00 + retries)**: 早上 07:00、07:20、07:40 检查，推送 **今天** 的课表。
*   **Night Push (20:00 + retries)**: 晚上 20:00、20:20、20:40 检查，推送 **明天** 的课表。

每组后两个时间是补发检查，不是重复轰炸。第一次成功后会保存场次记录，后续任务自动跳过；只有教务系统或云端临时失败时才继续尝试。

### 如何开启/关闭？

假设你想开启“晚安版”：
1.  点击左侧的 **Night Push (20:00 + retries)**。
...
1.  在你的仓库页面，点击 **Code** 选项卡。
2.  进入 `.github/workflows` 文件夹。
3.  点击你想修改的文件（例如 `morning_push.yml`）。
4.  点击右上角的 ✏️ (铅笔图标) 编辑。
5.  找到 `cron: '0 23 * * *'` 这一行。
    *   **注意**：这里的时间是 **UTC 时间**，比北京时间慢 8 小时。
    *   **计算公式**：`北京时间 - 8 = UTC 时间`。
    *   **例子**：如果你想**早上 06:30** 推送：
        *   6:30 - 8小时 = 前一天 22:30。
        *   写成：`cron: '30 22 * * *'`
