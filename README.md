...
配置保存在当前 Windows 用户目录的 `.ClassPush_HNIT` 文件夹中，与其他学校版本隔离。设置页里的“第 1 周周一”只是人工兜底，一般留空就行，让程序自己算。
...
- `Grade Check`：北京时间 `07:11～23:41` 每半小时检查一次成绩。
...
- 请保持合理检查频率。成绩默认每半小时检查一次，不建议改成五分钟催一次，老师不会因此录得更快。
